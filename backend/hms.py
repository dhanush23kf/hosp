...
def init_db():
    ...
    clinical_notes TEXT,
    vitals_json TEXT,
    token_fee INTEGER DEFAULT 300,   -- ✅ NEW
    doctor_fee INTEGER DEFAULT 0,    -- ✅ NEW
...
def update_appointment_status(apt_id):
    ...
    # update appointment
    conn.execute(
        'UPDATE appointments SET status=?, doctor_fee=? WHERE id=?',
        (new_status, doctor_fee, apt_id)
    )

    # 🔥 IF COMPLETED → SAVE TO HISTORY (OPD)
    if new_status == "Completed":
        ...
        token_fee = ap['token_fee']
        total = token_fee + doctor_fee

        conn.execute('''
            INSERT INTO admissions 
            (token_number, patient_name, patient_phone, problem, admit_date, total_bill)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (
            ap['token_number'],
            ap['patient_name'],
...
def admit_patient():
    ...
    try:
        # 🔍 CHECK IF PATIENT ALREADY EXISTS (OPD → IPD case)
        existing = conn.execute(
            'SELECT id FROM admissions WHERE token_number=? AND discharge_date IS NULL',
            (data['token_number'],)
        ).fetchone()

        if existing:
            # ✅ UPDATE EXISTING RECORD (convert OPD → IPD)
            conn.execute('''
                UPDATE admissions 
                SET 
                    patient_age=?,
                    ...
                    bed_id=?,
                    admit_date=?
                WHERE token_number=? AND discharge_date IS NULL
            ''', (
                data['age'],
                data['phone'],
                data['problem'],
            ...
            # ✅ NEW ENTRY (direct IPD admission)
            conn.execute('''
                INSERT INTO admissions 
...
def discharge_patient(bed_id):
    ...
    try:
        admission = conn.execute(
            'SELECT * FROM admissions WHERE bed_id=? AND discharge_date IS NULL',
            (bed_id,)
        ).fetchone()
        ...
        appointment = conn.execute(
            'SELECT token_fee, doctor_fee FROM appointments WHERE token_number = ?',
            (admission['token_number'],)
        ).fetchone()

        token_fee = appointment['token_fee'] if appointment else 300
        doctor_fee = appointment['doctor_fee'] if appointment else 0

        admit_time = datetime.fromisoformat(admission['admit_date'].replace('Z', ''))
        now = datetime.now()
        days_stayed = max(1, (now - admit_time).days)

        bed_data = conn.execute(
            'SELECT base_price FROM beds WHERE id=?',
            (bed_id,)
        ).fetchone()

        bed_total = days_stayed * bed_data['base_price']

        # ✅ FINAL BILL
        total_payable = token_fee + doctor_fee + bed_total

        conn.execute(
            'UPDATE admissions SET discharge_date=?, total_bill=? WHERE id=?',
